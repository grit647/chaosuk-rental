...
2, 3, 13 all confirmed matching between invoice form and device status
list after the fix).

**Context:** ต่อจากงาน "Water meter calibration" วันก่อนหน้า (ดู section
ด้านล่าง) — คุณต้นอธิบายสูตรที่ควรจะเป็นสำหรับห้องที่**ยังไม่ได้คาลิเบรต**
(สะสมของเราเองยังไม่ตรงกับ Tuya's real Total Use): "ค่ารวมที่ใช้คิดบิล =
สะสม(ของเราเอง, เริ่มนับจาก 0 ตอนเชื่อม Tuya) + SET NEW(ชดเชยว่า Tuya
เริ่มนับจาก 0 ไม่ใช่จากค่าเริ่มต้นจริง), หน่วยที่ใช้เดือนนี้ = ค่ารวม
(ปัจจุบัน) − waterPrev(บิลก่อนหน้า)"

**บั๊กที่แก้จริง — `deviceCharge()`'s water branch เดิมใช้สะสมดิบ (`live.
usage`) เป็นค่าตั้งต้นคำนวณ ไม่รวม SET NEW เลย** — ทำให้ห้องที่ยังไม่
คาลิเบรตแสดง "0.00 หน่วย" ทุกห้อง (สะสมดิบยังต่ำกว่า waterPrev อยู่ ลบ
แล้วติดลบโดน `Math.max(0,...)` clamp เป็น 0 ทั้งที่มีการใช้น้ำจริง) — แก้
ให้ใช้ `live.usage + (Number(room.waterSetNewValue) || 0)` เป็นค่าตั้งต้น
แทน (ห้องที่คาลิเบรตแล้ว SET NEW ถูกล้างเป็น '' อัตโนมัติหลังคาลิเบรต
สำเร็จอยู่แล้ว จึงลดเหลือแค่สะสมเฉยๆ พอดี ไม่กระทบห้องที่คาลิเบรตแล้ว)

**เจอบั๊กพลอย 1 อย่างระหว่างทาง — คำแนะนำ "SET NEW = waterPrev" ที่ให้ไป
ตอนแรกผิด:** ทำให้สูตรหักล้างกันเองพอดี `(สะสม + waterPrev×1000) −
waterPrev×1000 = สะสม เฉยๆ` — กลายเป็นโชว์ค่า Sum ดิบแทนหน่วยที่ใช้จริง
(เจอจากคุณต้นสังเกตว่าตัวเลขไม่สมเหตุสมผล) — **ทางแก้ที่ถูกต้องจริง: ใช้
ปุ่ม "🎯 คาลิเบรตมิเตอร์น้ำ" คาลิเบรตแต่ละห้องด้วยค่า Total Use จริงจากแอป
Tuya แทนการกรอก SET NEW เอง** (เขียนทับยอดสะสมให้ตรงกับ Tuya โดยตรง ไม่มี
ปัญหาหักล้างกัน) — คาลิเบรตครบทุกห้องแล้ว (ยกเว้นห้อง 5 ที่ waterPrev=0
ไม่ต้องคาลิเบรต)

**เจอบั๊กพลอย 2 อย่าง — กล่อง "เลขมิเตอร์น้ำ"/"เลขมิเตอร์ไฟ" ในฟอร์มออก
ใบแจ้งหนี้ (`invWaterDeviceDisplay`/`invElecDeviceDisplay`) ดึงค่าสะสมดิบ
(`live.usage`/`live.energy`) ตรงๆ คนละสูตรกับที่ใช้คิดบิลจริง** (`device
Charge()`, รวม SET NEW/waterPrev แล้ว) — ทำให้ตัวเลขที่โชว์บนจอไม่ตรงกับ
ที่ใช้คำนวณเงินจริงเบื้องหลัง คุณต้นจับได้ว่า "1281.46" ในกล่องไม่ตรงกับ
"56 หน่วย × 8 = 448 บาท" ที่อยู่ข้างล่าง — แก้ให้ทั้งคู่ดึงจาก `device
Charge().units` ตัวเดียวกับที่ใช้ออกบิลจริงแทน (กล่องนี้อ่านอย่างเดียว/
readonly ในโหมด "อุปกรณ์" อยู่แล้ว ถ้าต้องการแก้ไขค่าเองต้องสลับไปโหมด
"กรอกเอง")

**ฟีเจอร์เพิ่มเติมที่ทำในรอบเดียวกัน:**
- เพิ่ม **"หน่วยที่ใช้เดือนนี้"** แสดงในรายการ "สถานะอุปกรณ์ไฟฟ้า/น้ำ"
  (`buildDeviceRow`) — เดิมโชว์แค่เลขสะสมดิบ ไม่มีทางรู้ว่าใช้ไปเท่าไหร่
  จริงในรอบบิลนี้จากหน้านี้เลย — ดึงจาก `deviceCharge()` เดียวกับทุกจุด
  ตามคำขอปรับข้อความภายหลัง สลับให้ "X หน่วย/เดือนนี้" เป็นตัวใหญ่เด่น
  (เพราะเป็นตัวเลขที่ใช้คิดเงินจริง) ส่วน "X ลิตร (ผลรวมแบบ Tuya)" เป็น
  ตัวเล็กรอง (แค่ข้อมูลอ้างอิง)
- เปลี่ยนกล่อง "สะสม" ในหน้า Set อุปกรณ์ (รายละเอียดต่อห้อง,
  `equipSelLiveWaterUsage`) ให้แสดง "หน่วย/เดือนนี้" แทนสะสมดิบเช่นกัน
  (ตัวเดียวกับ `deviceCharge()`)
- **ซ่อนช่อง "ค่า SET NEW" ในหน้า Set อุปกรณ์แล้ว** (ครอบด้วย `sc-if
  value="{{ false }}"`, ไม่ได้ลบ field/logic เดิมออก) — ยืนยันแล้วว่า
  ปุ่มคาลิเบรตจริงเขียนทับยอดสะสมให้ตรงกับ Tuya โดยตรงทุกครั้ง ไม่ต้อง
  พึ่ง SET NEW ชดเชยส่วนต่างอีกต่อไปทั้งตอนนี้และอนาคต (คาลิเบรตรอบถัดไป
  ถ้าค่าเพี้ยนอีกก็แค่กดคาลิเบรตใหม่ด้วยค่า Total Use ปัจจุบัน ไม่มีส่วน
  ต่างค้างให้ต้องเก็บ)
- **`server/claudeTools.js`'s `calibrate_water_meters` (AI tool ในกล่อง
  คำสั่ง) เพิ่มการแจ้งเตือนไลน์อัตโนมัติ** ("ห้องไหนที่คาลิเบรตไม่ได้ ให้
  ส่งข้อความไลน์ไปหาเจ้าของ อาจติดสถานะใช้น้ำอยู่ ห้องไหนคาลิเบรตแล้วก็
  ส่งข้อมูลไปไลน์เจ้าของเหมือนกัน") — ส่งข้อความเดียวสรุปทั้ง ✅ ห้องที่
  สำเร็จ (ก่อน→หลัง, % ความแม่นยำเดิม) และ ❌ ห้องที่ล้มเหลว (พร้อมสาเหตุ)
  ไปยัง `adminLineUserId` ตรงๆ ไม่ผ่าน category gate ของ `notifyAdmin`
  (ส่งเสมอ ไม่ต้องมีสวิตช์แยก)

**สรุปคำศัพท์ที่ยืนยันชัดเจนในรอบนี้ (ตรงกับกฎ "เลขมิเตอร์ vs หน่วย" เดิม
ทุกประการ):** "สะสม"/เลขมิเตอร์ = ยอดสะสมตลอดชีพ (เหมือนเลขไมล์รถยนต์ วิ่ง
สะสมไปเรื่อยๆ ไม่รีเซ็ต, พอคาลิเบรตจะถูกเขียนทับใหม่ทั้งหมดให้ตรงกับ Tuya
ทันที) — "หน่วยที่ใช้เดือนนี้" = ยอดสะสมปัจจุบัน − ยอดบิลก่อนหน้า (delta,
ตัวเลขที่ใช้คิดเงินจริง) — 2 ค่านี้ไม่มีวันเท่ากัน (ต่างกันเท่ากับ
waterPrev พอดี) เป็นการออกแบบตั้งใจ ไม่ใช่บั๊ก
