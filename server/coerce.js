...
async function readSettings(preloadedRows) {
  ...
  return {
    ...
    // "ข้อนี้ทำสวิทย์ เปิดปิดเงื่อนไขนี้ไปก่อนครับ ตั้งเป็นปิดการทำงานไว้
    // ครับ" (2026-09-04) — เจ้าของอยากปิดกลไก resend ทั้งหมวดนี้ไว้ชั่วคราว
    // (หลังเจอว่าข้อความ "ส่งซ้ำสูงสุด 2 ครั้ง" ไม่ตรงกับที่โค้ดทำจริง แค่ 1
    // ครั้ง — ระหว่างรอตัดสินใจว่าจะแก้ยังไง ขอปิดไว้ก่อน) — default false
    // (ปิด) แม้แต่ตึกเดิมที่ไม่เคยตั้งค่านี้มาก่อนเลย ต่างจาก receiptRetryHours
    // ด้านบนที่ default เปิดอยู่แล้วที่ 24 ชม. — ตั้งใจให้ปิดทันทีทุกตึกจนกว่า
    // เจ้าของจะกลับมาเปิดเองที่หน้าตั้งค่า
    receiptRetryEnabled: bool(map.receiptRetryEnabled, false),
